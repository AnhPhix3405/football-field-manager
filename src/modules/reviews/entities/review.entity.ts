...
@Unique('UQ_field_reviews_booking_reviewer', ['bookingId', 'reviewerId'])
@Check('CHK_field_reviews_rating', 'rating >= 1 AND rating <= 5')
@Index('IDX_field_reviews_field_created', ['fieldId', 'createdAt'])
export class FieldReviewEntity {
  ...
  @Index('IDX_field_reviews_reviewer')
...
@Index('IDX_user_reviews_target_created', ['targetUserId', 'createdAt'])
export class UserReviewEntity {
  ...
  @Index('IDX_user_reviews_reviewer')
  @Column({ name: 'reviewer_id', type: 'uuid' })
  reviewerId: string;
...
export class FieldRatingSummaryEntity {
  ...
  @Index('IDX_field_rating_summary_avg')
  @Column({
    name: 'avg_rating',
    type: 'decimal',
    precision: 3,
...
export class UserRatingSummaryEntity {
  ...
  @Index('IDX_user_rating_summary_avg')
  @Column({
    name: 'avg_rating',
    type: 'decimal',
    precision: 3,
  ...
  @Column({ name: 'total_reviews', type: 'int', default: 0 })
  totalReviews: number;
  @Index('IDX_user_rating_summary_sportsmanship')
  @Column({
