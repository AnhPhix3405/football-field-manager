...
@Entity('users') export class UserEntity {
 ...
 @Index({unique:true,where:'phone IS NOT NULL'}) @Column({nullable:true,unique:true}) phone:string|null;
...
@Entity('user_profiles') export class UserProfileEntity {
 ...
 @Column({name:'full_name',nullable:true}) fullName:string|null;
 ...
 @Column({nullable:true}) gender:string|null;
}
@Entity('owner_profiles') export class OwnerProfileEntity {
 @PrimaryGeneratedColumn('uuid') id:string;
 @Index({unique:true}) @Column({name:'user_id',type:'uuid',unique:true}) userId:string;
 @Column({name:'business_name',nullable:true}) businessName:string|null;
 @Column({name:'business_license',nullable:true}) businessLicense:string|null;
 @Column({name:'bank_account',nullable:true,select:false}) bankAccount:string|null;
